...
export async function closeCurrentWeekAndCreateNext(supabase: SupabaseClient): Promise<{ closedReviewId: string; nextReviewId: string }> {
  ...
  const { data: nextReview, error: createError } = await supabase
    .from("weekly_reviews")
    .insert({ week_label: weekLabel, is_closed: false })
    .select("id")
    .single<{ id: string }>();

  if (createError || !nextReview) {
    throw createError ?? new Error("No se pudo crear la nueva semana actual.");
  }

  return {
    closedReviewId: reviewId,
